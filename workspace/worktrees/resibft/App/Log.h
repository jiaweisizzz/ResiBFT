...
private:
	...
	std::map<View, std::set<MsgPrepareProposalResiBFT>> prepareProposalsResiBFT;  // HotStuff Prepare Proposal
	std::map<View, std::set<MsgCommitResiBFT>> commitsResiBFT;  // HotStuff Commit

	// ResiBFT-specific: VC, BC, Committee messages
...
public:
	...
	// Basic ResiBFT
	// Return the number of signatures
	unsigned int storeMsgNewviewResiBFT(MsgNewviewResiBFT msgNewview);
	unsigned int storeMsgLdrprepareResiBFT(MsgLdrprepareResiBFT msgLdrprepare);
	unsigned int storeMsgPrepareResiBFT(MsgPrepareResiBFT msgPrepare);
	unsigned int storeMsgPrecommitResiBFT(MsgPrecommitResiBFT msgPrecommit);
	unsigned int storeMsgDecideResiBFT(MsgDecideResiBFT msgDecide);
	unsigned int storeMsgPrepareProposalResiBFT(MsgPrepareProposalResiBFT msgPrepareProposal);  // HotStuff
	unsigned int storeMsgCommitResiBFT(MsgCommitResiBFT msgCommit);  // HotStuff

	// Collect [n] signatures of the messages
	std::set<MsgNewviewResiBFT> getMsgNewviewResiBFT(View view, unsigned int n);
	Signs getMsgPrepareResiBFT(View view, unsigned int n);
	Signs getMsgPrecommitResiBFT(View view, unsigned int n);
	Signs getMsgDecideResiBFT(View view, unsigned int n);
	Signs getMsgCommitResiBFT(View view, unsigned int n);  // HotStuff

	// ResiBFT-specific: VC, BC, Committee
	// Store VC (Verification Certificate) messages
	unsigned int storeMsgVCResiBFT(MsgVCResiBFT msgVC);
	// Get VCs by type for a specific view
	std::set<MsgVCResiBFT> getMsgVCResiBFT(View view);
	std::set<MsgVCResiBFT> getMsgVCResiBFTByType(View view, VCType type);
	// Check if VC quorum reached for a type
	bool hasVCQuorum(View view, VCType type, unsigned int quorumSize);

	// Store BC (Block Certificate) messages
	unsigned int storeMsgBCResiBFT(MsgBCResiBFT msgBC);
	std::set<MsgBCResiBFT> getMsgBCResiBFT(View view);
	bool hasBCQuorum(View view, unsigned int quorumSize);

	// Store Committee messages
	unsigned int storeMsgCommitteeResiBFT(MsgCommitteeResiBFT msgCommittee);
	MsgCommitteeResiBFT getMsgCommitteeResiBFT(View view);  // Get the full MsgCommittee message
	Group getCommitteeResiBFT(View view);
	bool hasCommitteeQuorum(View view, unsigned int quorumSize);
