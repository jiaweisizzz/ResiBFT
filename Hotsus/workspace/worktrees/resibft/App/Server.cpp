...
#elif defined(BASIC_RESIBFT)
	long unsigned int sizeMessage = std::max({sizeof(MsgNewviewResiBFT), sizeof(MsgLdrprepareResiBFT), sizeof(MsgPrepareResiBFT), sizeof(MsgPrecommitResiBFT), sizeof(MsgDecideResiBFT), sizeof(MsgBCResiBFT), sizeof(MsgVCResiBFT), sizeof(MsgCommitteeResiBFT)});
#endif
...
#elif defined(BASIC_RESIBFT)
	ResiBFT resibft(keysFunctions, replicaId, numGeneralReplicas, numTrustedReplicas, numReplicas, numViews, numFaults, leaderChangeTime, nodes, privateKey, peerNetConfig, clientNetConfig);
#endif
